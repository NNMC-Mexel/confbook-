...
export default function Calendar({ rooms, onSlotClick, onBookingClick, onBookingsLoaded }) {
  ...
  const { bookings, refetch } = useBookings({ dateFrom, dateTo });
  ...
  function getBookingsForSlot(roomId, date, hour) {
    ...
    return bookings.filter((b) => {
      ...
      return bRoom === roomId && bDate === dateStr && hour >= startHour && hour < effectiveEnd;
    });
  }
            ...
            <div className="grid grid-cols-7">
              {days.map((day) => (
                <div key={day.toISOString()} className="border-l border-gray-100">
                  {HOURS.map((hour) => {
                    const occupied = getBookingsForSlot(rooms[0]?.id, day, hour).length > 0
                      || getBookingsForSlot(rooms[1]?.id, day, hour).length > 0;

                    return (
                      <div
                        key={hour}
                        className="h-16 border-b border-gray-50 relative group"
                      >
                        {rooms.map((room, idx) => {
                          ...
                          return starting.map((booking) => {
                            ...
                            return (
                              <div
                                ...
                                className={`absolute ${idx === 0 ? 'left-0.5' : 'left-1/2'} ${idx === 0 ? 'right-1/2' : 'right-0.5'} ${ROOM_BG_LIGHT[idx]} ${ROOM_BORDER[idx]} border rounded-lg px-1.5 py-0.5 cursor-pointer hover:shadow-md transition-shadow overflow-hidden z-10`}
                                style={{
                                  top: `${offsetFrac * 64}px`,
                                  height: `${spanHours * 64 - 2}px`,
                                }}
              ...
              return (
                <div key={hour} className="h-20 border-b border-gray-50 relative">
                  {rooms.map((room, idx) => {
                    ...
                    return starting.map((booking) => {
                      ...
                      return (
                        <div
                          key={booking.id || booking.documentId}
                          className={`absolute ${idx === 0 ? 'left-0.5 right-1/2' : 'left-1/2 right-0.5'} ${ROOM_BG_LIGHT[idx]} ${ROOM_BORDER[idx]} border rounded-lg px-2 py-1 cursor-pointer hover:shadow-md transition-shadow overflow-hidden z-10`}
                          style={{
                            top: `${offsetFrac * 80}px`,
                            height: `${spanHours * 80 - 2}px`,
                          }}
                          onClick={() => onBookingClick?.(booking)}
                        >
                          <div className={`text-[10px] font-semibold ${ROOM_TEXT_COLORS[idx]} truncate opacity-70`}>
                  ...
                  <div
                    ...
                    onClick={() =>
                      onSlotClick?.({
                        date: format(viewDay, 'yyyy-MM-dd'),
                        hour,
                      })
                    }
                  />
